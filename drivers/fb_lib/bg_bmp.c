...
#if 0//def INIT_PRINTF//def SKILL_FOR_SHUN_LISTCHANNEL_REPEATED //mllee 110824  
...
HBMP  open_bmp( const char *bmp_file, HBMP_i_t *hbmp_hdl_buf )
{
	...
	__msg("hdl->width=%d,hdl->height=%d,hdl->bitcount=%d,info_head.biSizeImage=%d\n",hdl->width,hdl->height,hdl->bitcount,info_head.biSizeImage);
...
__s32   get_file_head( HBMP hbmp ,bmp_file_head_t *file_head)
{
	HBMP_i hdl = (HBMP_i)hbmp;
	...
	eLIBs_memcpy((void *)file_head, g_pbmp_buf, sizeof(bmp_file_head_t));

	return EPDK_OK;
}


__s32   get_info_head( HBMP hbmp ,bmp_info_head_t *info_head)
{
	bmp_file_head_t  file_head;
	HBMP_i hdl = (HBMP_i)hbmp;
    ...
    eLIBs_memcpy((void *)info_head, (char*)(g_pbmp_buf+sizeof(file_head)), sizeof(bmp_info_head_t));

	return EPDK_OK;
}


__s32   get_data( HBMP hbmp ,void *data)
{
	bmp_file_head_t  file_head;
	bmp_info_head_t  info_head;
	HBMP_i hdl = (HBMP_i)hbmp;
	...
	eLIBs_memcpy(&file_head, g_pbmp_buf, sizeof(bmp_file_head_t));
    eLIBs_memcpy(&info_head, (char*)(g_pbmp_buf+sizeof(file_head)), sizeof(bmp_info_head_t));

	__msg("info_head.biSizeImage=%d\n",info_head.biSizeImage);
	__msg("sizeof(bmp_file_head_t)+sizeof(bmp_info_head_t)=%d\n",sizeof(bmp_file_head_t)+sizeof(bmp_info_head_t));
	__msg("hdl->matrix_off=%d\n",hdl->matrix_off);
    eLIBs_memcpy(data, (char*)(g_pbmp_buf+sizeof(bmp_file_head_t)+sizeof(bmp_info_head_t)), info_head.biSizeImage);
